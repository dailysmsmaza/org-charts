...
class Master extends CommonApi {
	...
	public function current_auth_key_get()
	{
		 $this->load->model('auth_model');
		 $auth_data = $this->auth_model->get_current_auth_key();
		 $auth_data_count = count($auth_data);
		 $this -> commonResponseJson( ($auth_data_count>0) ? 200 : 204, ($auth_data_count>0) ? "Success" : "No Auth Key Found.", $auth_data);
	}

	public function all_auth_key_get()
	{
		 $this->load->model('auth_model');
		 $auth_data = $this->auth_model->get_all_auth_key();
		 $auth_data_count = count($auth_data);
		 $this -> commonResponseJson( ($auth_data_count>0) ? 200 : 204, ($auth_data_count>0) ? "Success" : "No Auth Key Found.", $auth_data);
	}
