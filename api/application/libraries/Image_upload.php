...
//					}
	...
	protected function checkFileExist($checkFileExist,$name)
	{
		...
		//if(array_key_exists($checkFileExist,$this->upload_path) AND file_exists($this->upload_path[$checkFileExist].$name) ) {
			$this->new_file_name =  url_title(strtolower($file_info['filename'])).'1'.rand(1,9999999999) * 2 .".". $file_info['extension'];
		//}else{
			//$this->new_file_name =  url_title(strtolower($file_info['filename'])).'.'.$file_info['extension'] ;
		//}
