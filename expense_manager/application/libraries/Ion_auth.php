	...
	public function forgotten_password($identity)
	{
		if ($this->ion_auth_model->forgotten_password($identity))
		{
			...
			$this->email->to($user->email);
                                ...
                                $data['websetting'] = $this->setting->get(1);
                                $forgotten_password_code  = $user->forgotten_password_code;
                                //print_r($data['user']); exit();
                                $html_template=$this->email_template->get_email_template_by_id('1');
                               
				$message = $html_template->email_template_description;
				$message = str_replace('%project_name%', $data['websetting']->site_project_name, $message);
				$message = str_replace('%site_url%', base_url(), $message);
				$message = str_replace('%site_logo%', $data['websetting']->website_frontend_logo, $message);
                                $message = str_replace('%copyright%', $data['websetting']->site_copy_right, $message);
                                $message = str_replace('%username%', $user->username, $message);
				$message = str_replace('%uid%',base_url().'admin/reset-password/'. $forgotten_password_code,$message);
				$message = str_replace('%date%', date("F j, Y"), $message);
                                
			
				$this->email->subject($html_template->email_template_subject);
